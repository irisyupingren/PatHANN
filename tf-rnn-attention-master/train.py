...
X_train = np.array(sequence.pad_sequences(X_train, maxlen=max_review_length))
X_test = np.array(sequence.pad_sequences(X_test, maxlen=max_review_length))
...
embeddings_var = tf.Variable(tf.random_uniform([int(vocabulary_size), EMBEDDING_DIM], -1.0, 1.0), trainable=True)
# embeddings_var = Embedding(len(X_train), EMBEDDING_DIM, input_length=max_review_length)
